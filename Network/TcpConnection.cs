...
namespace Network
{
    ...
    public class TcpConnection : Connection
    {
        ...
        internal void EstablishUdpConnection(Action<IPEndPoint, IPEndPoint> connectionEstablished)
        {
            ...
            RegisterPacketHandler(typeof(EstablishUdpResponse), (packet, connection) =>
            {
                UnRegisterPacketHandler(typeof(EstablishUdpResponse));
                EstablishUdpResponse establishUdpResponse = (EstablishUdpResponse)packet;
                connectionEstablished.Invoke(udpEndPoint, new IPEndPoint(IPRemoteEndPoint.Address, establishUdpResponse.UdpPort));
                Send(new EstablishUdpResponseACK());
            });

            Send(new EstablishUdpRequest(udpEndPoint.Port));
        }
        ...
        protected override byte[] ReadBytes(int amount)
        {
            ...
            while (socket.Available < amount)
                Thread.Sleep(CPU_SAVE);

            byte[] requestedBytes = new byte[amount];
            client.GetStream().Read(requestedBytes, 0, amount);
        ...
        protected override void WriteBytes(byte[] bytes)
        {
            ...
            if (ForceFlush) client.GetStream().Flush();
        }
